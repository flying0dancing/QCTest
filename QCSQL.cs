...
order by RT_FROM_REQ_ID";
        public static string SQL_QC_Req_3800 = @"select * from
(
SELECT
A.RQ_REQ_PATH  AS 'Hierarchy',
A.RQ_REQ_ID AS 'Legacy ID',
...
where Hierarchy like (SELECT RQ_REQ_PATH FROM REQ WHERE RQ_REQ_NAME='Design Input - Prague')+'%' or  Hierarchy like (SELECT RQ_REQ_PATH FROM REQ WHERE RQ_REQ_NAME='Design Input IS 3800 PCBA tool')+'%'
order by Hierarchy,RQ_ORDER_ID";
        public static string SQL_QC_TestCases_3800 = @"select * from
(
select
CASE WHEN A.TS_SUBJECT=-2 THEN (select replace(min(AL_ABSOLUTE_PATH),'AAA','XXX')+'111' from ALL_LISTS where AL_FATHER_ID not in ('-1', '0') ) ELSE ALL_LISTS.AL_ABSOLUTE_PATH+'111' END   As 'Hierarchy',
A.TS_NAME As 'Name',
...
where Hierarchy like (SELECT AL_ABSOLUTE_PATH FROM ALL_LISTS where AL_DESCRIPTION = 'Firmware' )+'%' or Hierarchy like (SELECT AL_ABSOLUTE_PATH FROM ALL_LISTS where AL_DESCRIPTION = 'Hardware' )+'%' or Hierarchy like (SELECT AL_ABSOLUTE_PATH FROM ALL_LISTS where AL_DESCRIPTION = 'MFG Tool & Fixture' )+'%' or Hierarchy like (SELECT AL_ABSOLUTE_PATH FROM ALL_LISTS where AL_DESCRIPTION = 'PCBA Tool' )+'%'
order by  Hierarchy, Name, Step#";
        public static string SQL_QC_TCTrace_3800 = @"SELECT
REQ_COVER.RC_REQ_ID As 'QC_ReqID',
(select RQ_REQ_NAME from REQ where REQ_COVER.RC_REQ_ID=REQ.RQ_REQ_ID ) As 'QC_ReqName',
REQ_COVER.RC_ENTITY_ID As 'QC_TCID',
(SELECT TEST.TS_NAME FROM TEST where REQ_COVER.RC_ENTITY_ID=TEST.TS_TEST_ID) As 'QC_TCName'
FROM  REQ_COVER
where RC_REQ_ID in (Select RQ_REQ_ID from REQ where RQ_REQ_PATH like (SELECT RQ_REQ_PATH FROM REQ WHERE RQ_REQ_NAME='Design Input - Prague' )+'%' or RQ_REQ_PATH like (SELECT RQ_REQ_PATH FROM REQ WHERE RQ_REQ_NAME='Design Input IS 3800 PCBA tool' )+'%')
order by RC_REQ_ID,RC_ENTITY_ID";

        public static string SQL_QC_TestCase_EnvistaImaging = @"select
CASE WHEN A.TS_SUBJECT=-2 THEN (select replace(min(AL_ABSOLUTE_PATH),'AAA','XXX')+'111' from ALL_LISTS where AL_FATHER_ID not in ('-1', '0') ) ELSE ALL_LISTS.AL_ABSOLUTE_PATH+'111' END   As 'Hierarchy',
A.TS_NAME As 'Name',
A.TS_DESCRIPTION As 'Description',
Case When AL_ABSOLUTE_PATH like (select AL_ABSOLUTE_PATH from ALL_LISTS where AL_DESCRIPTION ='_Obsolete')+'%' Then 'Obsolete'
...
A.DS_EXPECTED As 'Step Expected Result',
CASE WHEN DS_ATTACHMENT='Y' THEN 'HasAttachment' ELSE '' END As 'Step Notes',
Case When A.TS_TYPE='VAPI-XP-TEST' Then 'Automated' Else A.TS_TYPE End as 'Verification Method',
CONVERT(VARCHAR, A.TS_TEST_ID) As 'Legacy Trace',
A.TS_RESPONSIBLE /*Test.Designer*/ As 'Assigned',
CASE WHEN A.TS_STATUS is not Null Then ALL_LISTS.AL_ITEM_ID ELSE ALL_LISTS.AL_FATHER_ID END As 'AL_FATHER_ID',
CASE WHEN A.TS_SUBJECT=-2 THEN 'Unattached' ELSE ALL_LISTS.AL_DESCRIPTION END As 'Folder_DESCRIPTION',/*for identify Unattached*/
